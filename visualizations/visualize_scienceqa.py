...
# -*- coding: utf-8 -*-
"""
Generate professional visualization for ScienceQA VLM experiments
"""
import json
...
def load_metrics(method):
    ...
    path = f'./results/llava-scienceqa-{method}-v2/metrics_{method}.json'
    with open(path) as f:
        return json.load(f)

def load_eval_results(method):
    """Load evaluation results"""
    path = f'./results/llava-scienceqa-{method}-v2/eval_results.json'
    with open(path) as f:
        return json.load(f)

def plot_scienceqa_comparison():
    ...
    # ============================================================
    # 2. Top Right: Final Accuracy (instead of Perplexity)
    # ============================================================
    ...
    for method in METHODS:
        ...
        accuracy = eval_results['accuracy'] * 100  # Convert to percentage
        accuracies.append(accuracy)
        labels.append(METHOD_NAMES[method])
        colors_list.append(COLORS[method])

    bars = ax2.bar(labels, accuracies, color=colors_list,
    ...
    ax2.set_title('Final Accuracy (Higher is Better)', fontweight='bold', pad=15)
    ...
    # Main title
    # ============================================================
    fig.suptitle('ScienceQA Visual Question Answering - Method Comparison',
                 fontsize=18, fontweight='bold', y=0.98)

    # Save figure
    output_path = './visualizations/scienceqa_professional.png'
...
if __name__ == '__main__':
    print('='*60)
    print('Generating ScienceQA Visualization')
    print('='*60)
    print()

    output_path = plot_scienceqa_comparison()
